...
To answer this, we did a common garden experiment: 8 populations, XX degree latitudinal gradient, grown in greenhouse for three years, common garden for 7-8 years, measured survival, growth, fertility, then burnt to assess re-sprouting ability.
...
More compacts growth forms in the north Earlier sexual recruitment in the north (one mode of reprod. only)
Seedling drought etc tolerance higher in the south (i.e., lower mortality)
Figures, tables, tests: Site map with heathland belt and Lygra and Tarva inserted (example plot of vegetative and seed recruitment after e.g., 2 yrs)
Growth through life (height\~time\*pop rand=mother, or a composite; and also report correlation of the size parameters) Growth form through life (#branches/height) form\~time\*pop rand=mother split inside/out Biomass (simple box plot per pop) Flowering\~size\*pop logistic Mortality\~size\*pop logistic Respouting\~pop -->

# Methods

_Calluna vulgaris_ seeds were collected from eight sites spanning the latitudinal, and climatic, gradient of Norwegian coastal heathlands in MONTH 20XX (@tbl-sites, @fig-site-map). 
...
```{r}
#| label: tbl-sites
#| tbl-cap: Locations seeds were collect from. GGD5 is growing degrees days above 5°C (@Brun2022; @Brun2022a)
#| eval: false
#| include: false
tar_read(metadata) |> 
  arrange(desc(Lat)) |> 
  gt() |> 
  cols_label(Lat = "Latitude °N", Long = "Longitude °E") |> 
...
GREENHOUSE DETAILS
X seeds from each maternal plant were sown. 
...
COMMON GARDEN DETAILS
After XX years, a random selection of the surviving plants were planted in a common garden.
...
All numerical analyses were done in `r stringr::str_remove(R.version.string, " \\(.*")` [@R2023].
